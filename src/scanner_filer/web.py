...
def _default_smb_cred_file(name: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "_", name).strip("_") or "user"
    return str(Path.home() / f".smb-nas-{safe}")
...
def create_app(cfg: AppConfig, config_path: Path) -> Flask:
    ...
    def setup_user_inbox_add():
        ...
        cred_file = _default_smb_cred_file(name)
    ...
    def setup_user_inbox_update():
        ...
        cred_file = _default_smb_cred_file(new_name)
